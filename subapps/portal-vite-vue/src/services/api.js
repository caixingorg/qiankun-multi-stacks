function createRequestExecutor(viewModel = {}) {
  if (typeof viewModel.requestClient === 'function') {
    return (path, payload = {}) => viewModel.requestClient(path, payload);
  }

  return async (path, payload = {}) => {
    const response = await fetch(path, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-runtime-channel': viewModel.runtimeChannel || 'stable',
      },
      body: JSON.stringify(payload),
    });

    return {
      ok: response.ok,
      status: response.status,
      path,
      payload,
    };
  };
}

export function getPortalApi(viewModel = {}) {
  const runtimeChannel = viewModel.runtimeChannel || 'stable';
  const request = createRequestExecutor(viewModel);
  const endpoints = {
    home: '/workspace',
    detail: '/announcements',
  };

  return {
    serviceName: 'portal-workspace-service',
    basePath: '/api/portal/workspace',
    runtimeChannel,
    transport: viewModel.requestTransport || 'browser-fetch',
    endpoints,
    fetchHomeData() {
      return request('/api/portal/workspace' + endpoints.home, {
        intent: 'portal-workspace',
        runtimeChannel,
      });
    },
    fetchDetailData() {
      return request('/api/portal/workspace' + endpoints.detail, {
        intent: 'portal-announcements',
        runtimeChannel,
      });
    },
  };
}
